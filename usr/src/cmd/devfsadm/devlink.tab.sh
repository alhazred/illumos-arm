...
case "$MACH" in
  ...
  "alpha" )
	#
	# These are the alpha specific entries
	# It depends on the build machine being an alpha
	#
	cat <<-EOM
	EOM
	;;
  ...
  "riscv64" )
	#
	# These are the riscv64 specific entries
	# It depends on the build machine being an riscv64
	#
	cat <<-EOM
	EOM
	;;
