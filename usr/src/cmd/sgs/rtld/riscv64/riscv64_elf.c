...
/*
 * riscv64 machine dependent and ELF file class dependent functions.
...
elf_bndr(Rt_map *lmp, ulong_t reloff)
{
	...
	{
		char buf[256];
		sprintf(buf,"%s: %lx pid=%d %s\n",PATHNAME(lmp),ADDR(lmp),getpid(), name);
		write(-2,buf,256);
	}
#endif
...
{
	return (conv_reloc_RISCV_type(rel));
}
