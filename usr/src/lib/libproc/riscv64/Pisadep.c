...
{
	P->status.pr_lwp.pr_reg[REG_T0] = sysindex;
    ...
    uintptr_t ap)
{
	...
	for (i = 0, adp = argp; i < nargs; i++, adp++) {
		...
		case 5:
			...
			break;
		case 6:
			(void) Pputareg(P, REG_A6, adp->arg_value);
			break;
		case 7:
			(void) Pputareg(P, REG_A7, adp->arg_value);
			break;
		}
	}
    ...
    uintptr_t ap)
{
	...
	for (i = 0, adp = argp; i < nargs; i++, adp++) {
		...
		case 6:
			adp->arg_value =
			    P->status.pr_lwp.pr_reg[REG_A6];
			break;
		case 7:
			adp->arg_value =
			    P->status.pr_lwp.pr_reg[REG_A7];
			break;
		}
	}
