...
#define STUB_COMMON(module, fcnname, install_fcn, retfcn, weak)		\
	ENTRY(fcnname);							\
	LDGP(pv);							\
	lda	t0, fcnname##_info;					\
	ldl	t1, MODS_FLAG(t0);					\
	beq	t1, 1f;			/* not weak */			\
	and	t1, MODS_INSTALLED, t2;	/* installed? */		\
	bne	t2, 1f;			/* yes, do the mod_hold */	\
	ldq	pv, MODS_RETFCN(t0);					\
	jsr	zero, (pv);		/* no, just jump to retfcn */	\
1:									\
	jsr	zero, stubs_common_code;				\
	SET_SIZE(fcnname);						\
...
#define STUB_UNLOADABLE(module, fcnname, install_fcn, retfcn, weak)	\
	...
	and	t1, MODS_INSTALLED, t2;	/* installed? */		\
	bne	t2, 1f;			/* yes */			\
	and	t1, MODS_WEAK, t2;	/* weak? */			\
	beq	t2, 2f;			/* no, load module */		\
	ldq	pv, MODS_RETFCN(t0);	/* yes, just jump to retfcn */	\
	jsr	zero, (pv);						\
1:									\
	ldq	pv, MODS_INSTFCN(t0);	/* just jump to install_fcn */	\
	jsr	zero, (pv);						\
2:									\
	jsr	zero, stubs_common_code;				\
	SET_SIZE(fcnname);						\
 ...
 */
	...
	mov	t0, a0
	CALL(mod_hold_stub)		/* mod_hold_stub(xxx_info) */
	lda	t0, 1(v0)		/* error ? */
	bne	t0, 1f			/* no */
...
1:
	...
	ldq	a0, 8*0(sp)	/* Restore original args and info */
	ldq	a1, 8*1(sp)
	ldq	a2, 8*2(sp)
	ldq	a3, 8*3(sp)
	ldq	a4, 8*4(sp)
	...
	lda	sp, -8*(MAXNARG-6+2)(sp)
	lda	t1, 0(sp)
	.rept	(MAXNARG - 6)
	ldq	t2, 8*(MAXNARG-6+2)(t1)
	stq	t2, 0(t1)
	lda	t1, 8(t1)
	.endr
	stq	s0, 8*(MAXNARG-6)(sp)
	stq	ra, 8*(MAXNARG-6+1)(sp)

	mov	t0, s0

	/* Call stub function */
	ldq	pv, MODS_INSTFCN(t0)
	jsr	ra, (pv)
	LDGP(ra)

	/* Preserve any return values */
	mov	s0, a0
	mov	v0, s0			/* Save return value */
	CALL(mod_release_stub)		/* release hold on module */

	/* Restore stack state and return. */
	mov	s0, v0
	ldq	s0, 8*(MAXNARG-6)(sp)
	ldq	ra, 8*(MAXNARG-6+1)(sp)
	lda	sp, 8*(MAXNARG-6+2)(sp)
	ret
 ...
 */
	...
	MODULE(platmod, misc);
	NO_UNLOAD_STUB(platmod, mach_init, nomod_void);
