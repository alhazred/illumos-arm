	...
	if (PTOU(p)->u_systrap) {
		if (prismember(&PTOU(p)->u_entrymask, code)) {
			...
			if (PTOU(p)->u_systrap &&
				...
				lwp->lwp_argsaved = 0;
				lwp->lwp_ap = (long *)&lwptoregs(lwp)->r_a0;
			}
			mutex_exit(&p->p_lock);
		}
		repost = 1;
...
/*
 * Execute a system call on behalf of the current thread.
 */
void
dosyscall(struct regs *rp)
{
	struct sysent	*se;
	rval_t		rval;
	kthread_t	*t = curthread;
	...
	ASSERT(lwp->lwp_ap == (long *)&lwptoregs(lwp)->r_a0);
	ASSERT(lwp->lwp_argsaved == 0);

	syscall_mstate(LMS_USER, LMS_SYSTEM);

	CPU_STATS_ENTER_K();
	CPU_STATS_ADDQ(t->t_cpu, sys, syscall, 1);
	CPU_STATS_EXIT_K();
	...
	se = code >= NSYSCALL ? &nosys_ent : LWP_GETSYSENT(lwp) + code;

#if 0
	if (ttoproc(t) && ttoproc(t)->p_pid >= 100020)
	{
		prom_printf("%s(): %d code=%d  %lx %lx %lx %lx %lx %lx\n",
		    __FUNCTION__, __LINE__, code,
		    rp->r_a0, rp->r_a1, rp->r_a2, rp->r_a3, rp->r_a4, rp->r_a5);
		//prom_printf("%s(): %d  sy_narg=%d sy_flags=%d sy_call=%p sy_callc=%p\n",
		//    __FUNCTION__, __LINE__, se->sy_narg, se->sy_flags, se->sy_call, se->sy_callc);
	}
#endif
	if (se->sy_narg >= 4 || (se->sy_flags & SE_LOADABLE)) {
		if (save_syscall_args() != 0) {
			set_errno(EFAULT);
			goto err;
		}
	}
	...
	if ((t->t_pre_sys | syscalltrace) != 0) {
		if (pre_syscall() !=  0)
			se = &sysent_err;	/* use dummy handler */
	}

	rval.r_vals = (*se->sy_callc)(rp->r_a0, rp->r_a1, rp->r_a2, rp->r_a3, rp->r_a4, rp->r_a5);

err:
#if 0
	prom_printf("%s(): %d rval.r_vals=%ld t->t_post_sys_ast=%d lwp->lwp_errno=%d\n",
	    __FUNCTION__, __LINE__, rval.r_vals, t->t_post_sys_ast, lwp->lwp_errno);
#endif
	/*
	...
	if ((t->t_post_sys_ast | syscalltrace) == 0) {
		 ...
		 */
		if (se->sy_flags & SE_32RVAL2) {
			rp->r_v0 = rval.r_v.r_v1;
			rp->r_t2 = rval.r_v.r_v2;
		} else {
			rp->r_v0 = rval.r_vals;
		}
		rp->r_t1 = 0;
		...
		if (se->sy_flags & SE_32RVAL2)
			post_syscall(rval.r_v.r_v1, rval.r_v.r_v2);
		else
			post_syscall(rval.r_vals, 0);
	}
...
/*
 * Indirect syscall handled in libc on Alpha architectures
 */
int64_t
indir()
{
