...
#define	MUTEX_GETTICK()	(__builtin_alpha_rpcc() & 0xffffffff)
...
{
	uintptr_t owner = __atomic_load_n(&lp->m_owner, __ATOMIC_ACQUIRE);
	while (owner != 0 && !__atomic_compare_exchange_n(&lp->m_owner, &owner, (owner | 0x1), 1, __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE)) {}
}
}
