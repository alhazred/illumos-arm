	...
	caddr_t	ekernelheap)
{
	...
	while (va != 0) {
		...
		if (va == VPT_BASE) {
			...
			va += VPT_SIZE;
			ptbl[l] += VPT_SIZE / LEVEL_SIZE(l);
			continue;
		}
		if (va == KSEG_BASE) {
			ASSERT(l == mmu.max_level);
			va += KSEG_SIZE;
			ptbl[l] += KSEG_SIZE / LEVEL_SIZE(l);
			continue;
		}
...
hat_kern_setup(void)
{
	 ...
	 */
	htable_attach(kas.a_hat, KERNELBASE, mmu.max_level, NULL, pfn);
...
boot_reserve(void)
{
	...
	ASSERT(is_reserved_memory((uintptr_t)get_l1ptbl() - KSEG_BASE));
	...
	while (va != 0) {
		...
		if (va == VPT_BASE) {
			ASSERT(l == mmu.max_level);
			va += VPT_SIZE;
			ptbl[l] += VPT_SIZE / page_size;
			continue;
		}
		if (va == KSEG_BASE) {
			ASSERT(l == mmu.max_level);
			va += KSEG_SIZE;
			ptbl[l] += KSEG_SIZE / page_size;
			continue;
		}
