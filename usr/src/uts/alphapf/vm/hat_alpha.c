...
static inline void hat_pcb_reload(pcb_t *pcb)
{
	register unsigned long a0 __asm__("$16");
	register unsigned long a1 __asm__("$17");

	a0 = pcb->pcb_self;
	a1 = (unsigned long)&pcb->pcb_hw.ksp;
	__asm__ __volatile__(
		"stq $30, 0($17);"
		"call_pal %1"
		: "=r"(a0)
		: "i"(PAL_swpctx), "r"(a0), "r"(a1)
 ...
 */
void
hat_thread_switch(kthread_t *thd)
{
	//	prom_printf("%s(): %p entry=%p tid=%d\n", __FUNCTION__, thd, thd->t_startpc, thd->t_tid);
	uint64_t ipl = pal_swpipl(6);
	hat_t *hat = thd->t_procp->p_as->a_hat;
	cpu_t *cpu = CPU;
	hat_t *old = cpu->cpu_current_hat;
	processorid_t cpuid = cpu->cpu_id;
	pcb_t *pcb = &ttolwp(thd)->lwp_pcb;
	...
	pcb->pcb_hw.asn = hat->hat_asid[cpuid];
	__sync_synchronize();
...
hati_mkpte(pfn_t pfn, uint_t attr, level_t level, uint_t flags)
{
	...
	switch (attr & (PROT_WRITE | PROT_USER)) {
	case PROT_USER | PROT_WRITE:
		PTE_SET(pte, PTE_KWE | PTE_URE | PTE_UWE);
		break;
	case PROT_WRITE:
		PTE_SET(pte, PTE_KWE);
		break;
	...
	if (flags & HAT_LOAD_NOCONSIST)
		PTE_SET(pte, PTE_NOCONSIST);
	else if (attr & HAT_NOSYNC)
		PTE_SET(pte, PTE_NOSYNC);
	else
		PTE_SET(pte, PTE_FOR | PTE_FOW);
...
hati_sync_pte_to_page(page_t *pp, pte_t pte, level_t level)
{
	...
	switch (PTE_GET(pte, PTE_FOR | PTE_FOW | PTE_KWE)) {
	case PTE_KWE:
		rm |= (P_REF | P_MOD);
		break;
	case 0:
	case PTE_FOW:
	case PTE_KWE | PTE_FOW:
		rm |= P_REF;
		break;
	default:
		break;
...
#define	PTE_REMAP_BITS							\
	(PTE_PFN_MASK | PTE_PROT_MASK | PTE_FLT_MASK | PTE_GH_MASK)
...
hat_unlock_region(struct hat *hat, caddr_t addr, size_t len,
    hat_region_cookie_t rcookie)
{
	panic("No shared region support on Alpha");
}
...
hat_getattr(hat_t *hat, caddr_t addr, uint_t *attr)
{
	...
	*attr = PROT_READ;
	if (PTE_GET(pte, PTE_KWE))
		*attr |= PROT_WRITE;
	if (PTE_GET(pte, PTE_URE))
		*attr |= PROT_USER;
	if (!PTE_GET(pte, PTE_FOE))
		*attr |= PROT_EXEC;
	if (PTE_GET(pte, PTE_SOFTWARE) >= PTE_NOSYNC)
		*attr |= HAT_NOSYNC;
	htable_release(ht);
...
try_again:
		...
		switch (what) {
		case HAT_LOAD_ATTR:
			PTE_CLR(newpte, PTE_KWE | PTE_UWE | PTE_SOFTWARE);
			PTE_SET(newpte, PTE_FOE);
		case HAT_SET_ATTR:
			if (attr & PROT_WRITE) {
				PTE_SET(newpte, PTE_KWE);
				if (PTE_GET(newpte, PTE_URE))
					PTE_SET(newpte, PTE_UWE);
			}
			if (attr & PROT_EXEC)
				PTE_CLR(newpte, PTE_FOE);
			if (attr & HAT_NOSYNC)
				PTE_SET(newpte, PTE_NOSYNC);
			break;
		case HAT_CLR_ATTR:
			if (attr & PROT_WRITE)
				PTE_CLR(newpte, PTE_KWE | PTE_UWE);
			if (attr & PROT_EXEC)
				PTE_SET(newpte, PTE_FOE);
			if (attr & HAT_NOSYNC)
				PTE_CLR(newpte, PTE_SOFTWARE);
			break;
		}
...
{
	return (((*((unsigned long *)VPT_BASE + VPT_IDX(va)))>>32)<<MMU_PAGESHIFT) | (va & MMU_PAGEOFFSET);
}

uint64_t hat_getptbr(struct hat *hat)
{
	ASSERT(hat != NULL);
	ASSERT(hat->hat_htable != NULL);
	return hat->hat_htable->ht_pfn;
}

void
hat_set_pcb(hat_t *hat, pcb_t *pcb)
{
	pcb->pcb_self = get_pa((uint64_t)&pcb->pcb_hw);
	pcb->pcb_hw.ptbr = hat_getptbr(hat);
}
...
hat_setup(hat_t *hat, int flags)
{
	...
	hat_set_pcb(hat, &lwp->lwp_pcb);
	hat_thread_switch(curthread);
	kpreempt_enable();
}
...
hati_update_pte(htable_t *ht, uint_t entry, pte_t expected, pte_t new)
{
	...
	if (PTE_GET(expected, PTE_SOFTWARE) < PTE_NOSYNC &&
	    !PTE_GET(expected, PTE_FOR) &&
	    (PTE_GET(new, PTE_NOSYNC) || !PTE_GET(new, PTE_KWE) ||
	    PTE_GET(new, PTE_FOR))) {

		pp = page_numtopp_nolock(PTE2PFN(expected, ht->ht_level));
		ASSERT(pp != NULL);
		switch (PTE_GET(expected, PTE_FOR | PTE_FOW | PTE_KWE)) {
		case PTE_KWE:
			rm |= (P_REF | P_MOD);
			break;
		case 0:
...
int
hat_page_falt(hat_t *hat, caddr_t vaddr, int cause)
{
	int rv = -1;
	...
	for (;;) {
		...
		if (PTE_GET(oldpte, PTE_FOR | PTE_FOW)) {
			...
			if (cause == 1) {
				PTE_CLR(newpte, PTE_FOR | PTE_FOW);
			} else {
				PTE_CLR(newpte, PTE_FOR);
			}
