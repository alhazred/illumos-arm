...
unlink_ptp(htable_t *higher, htable_t *old, uintptr_t vaddr, boolean_t is_kernel)
{
	...
	pte_t	expect = MAKEPTP(old->ht_pfn, old->ht_level);
	pte_t	found;
...
{
	uint_t	entry = htable_va2entry(vaddr, higher);
	pte_t	newptp = MAKEPTP(new->ht_pfn, new->ht_level);
	pte_t	found;
	...
	paddr_t pfn)
{
	 ...
	 */
	ptep = PT_INDEX_PTR(hat_kpm_pfn2va(ht->ht_pfn) + ((ht->ht_vaddr >> LEVEL_SHIFT(ht->ht_level)) & ((1<<NPTESHIFT)-1)) * sizeof(pte_t), 0);
	for (i = 0; i < HTABLE_NUM_PTES(ht); ++i) {
		bool in_vpt = (VPT_BASE <= base && base < VPT_BASE + VPT_SIZE - 1);
		bool in_kseg = (KSEG_BASE <= base && base < KSEG_BASE + KSEG_SIZE - 1);
		if (!in_vpt && !in_kseg) {
			pte = ptep[i];
			if (PTE_ISVALID(pte)) {
				++ht->ht_valid_cnt;
				if (!PTE_ISPAGE(pte, level)) {
					htable_attach(hat, base, level - 1, ht, PTE2PFN(pte, level));
				}
			}
		}
...
htable_scan(htable_t *ht, uintptr_t *vap, uintptr_t eaddr)
{
	 ...
	 */
	pte_ptr = (caddr_t)PT_INDEX_PTR(hat_kpm_pfn2va(ht->ht_pfn) + ((ht->ht_vaddr >> LEVEL_SHIFT(ht->ht_level)) & ((1<<NPTESHIFT)-1)) * sizeof(pte_t), 0);
	...
	uintptr_t eaddr)
{
	...
	if (hat->hat_ism_pgcnt > 0) {
		max_mapped_level = mmu.max_level;
	} else {
		max_mapped_level = 0;
		for (l = 1; l <= mmu.max_level; ++l)
...
htable_getpage(struct hat *hat, uintptr_t vaddr, uint_t *entry)
{
	...
	ht = htable_getpte(hat, vaddr, &e, &pte, mmu.max_level);
	if (ht == NULL)
		return (NULL);
...
pte_get(htable_t *ht, uint_t entry)
{
	 ...
	 */
	ASSERT(entry < HTABLE_NUM_PTES(ht));
	ptep = PT_INDEX_PTR(hat_kpm_pfn2va(ht->ht_pfn) + ((ht->ht_vaddr >> LEVEL_SHIFT(ht->ht_level)) & ((1<<NPTESHIFT)-1)) * sizeof(pte_t), entry);
	pte = GET_PTE(ptep);
	return (pte);
}
...
pte_copy(htable_t *src, htable_t *dest, uint_t entry, uint_t count)
{
	 ...
	 */
	dst_va = (caddr_t)PT_INDEX_PTR(hat_kpm_pfn2va(dest->ht_pfn) + ((dest->ht_vaddr >> LEVEL_SHIFT(dest->ht_level)) & ((1<<NPTESHIFT)-1)) * sizeof(pte_t), entry);
	src_va = (caddr_t)PT_INDEX_PTR(hat_kpm_pfn2va(src->ht_pfn) + ((src->ht_vaddr >> LEVEL_SHIFT(src->ht_level)) & ((1<<NPTESHIFT)-1)) * sizeof(pte_t), entry);
	/*
	 * now do the copy
	 */
	size = count << PTE_BITS;
