...
thread_load(kthread_t *t, void (*start)(), caddr_t arg, size_t len)
{
	...
	*--p = 0; // initial frame fp
	*--p = 0; // initial frame pc
	*--p = 0; // dummy
	*--p = (intptr_t)start;
	*--p = (long)len;
	*--p = (intptr_t)arg;
...
{
	lwptoregs(lwp)->r_t0 = 0;
	lwptoregs(lwp)->r_a0 = v1;
	lwptoregs(lwp)->r_a1 = v2;
}
...
setregs(uarg_t *args)
{
	 ...
	 */
	/* need not save sysycall registers because of num of argment is 8 in RISC-V */
	rp = lwptoregs(lwp);
	sp = rp->r_sp;
	bzero(rp, sizeof (*rp));
