...
#define	MUTEX_GETTICK()	csr_read_time()
...
{
	uintptr_t owner = __atomic_load_n(&lp->m_owner, __ATOMIC_ACQUIRE);
...
__MUTEX_SET_WAITERS(volatile mutex_impl_t *lp)
{
	...
	do {
		uintptr_t owner;
		asm volatile ("lr.d %0, %1":"=r"(owner):"m"(lp->m_owner):"memory");
		if (owner == 0) {
			break;
		}
		asm volatile ("sc.d %0, %2, %1":"=r"(result),"+m"(lp->m_owner):"r"(owner | MUTEX_WAITERS):"memory");
	} while (result);
}
...
{
	__atomic_store_n(&lp->m_owner, 0, __ATOMIC_RELEASE);
}

static inline void
__MUTEX_DESTROY(volatile mutex_impl_t *lp)
{
	__atomic_store_n(&lp->m_owner, ((uintptr_t)curthread | MUTEX_DEAD), __ATOMIC_RELEASE);
}
