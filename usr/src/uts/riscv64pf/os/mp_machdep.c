...
{
	return __builtin_ctzl(cpu->cpu_hartmask);
}
...
cpu_halt(void)
{
	...
	while (*p == 0 &&
		...
		old = csr_read_clear_sstatus(SSR_SIE);
	}
...
//	disp_enq_thread = cpu_wakeup;
	...
	uint64_t hartmask = enable_hartmask;
	for (int cpu_id = 0; cpu_id < NCPU && hartmask != 0; cpu_id++) {
