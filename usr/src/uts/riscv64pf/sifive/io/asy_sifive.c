...
#define	ASY_UART_CLK	(500 * 1000 * 1000)
...
{
	REG_WRITE(asy, UART_TXDATA, val);
}

static bool
meson_is_busy(struct asycom *asy)
{
	union uart_txdata reg;
	reg.dw = REG_READ(asy, UART_TXDATA);
	return reg.full;
}
...
{
	union uart_ie reg;
	reg.dw = REG_READ(asy, UART_IE);
	if (mask & RIEN) {
		reg.rxwm = ((icr & RIEN)? 1: 0);
	}
	if (mask & TIEN) {
		reg.txwm = ((icr & TIEN)? 1: 0);
	}
	REG_WRITE(asy, UART_IE, reg.dw);
}

static uint8_t
meson_get_char(struct asycom *asy)
{
	return REG_READ(asy, UART_RXDATA);
}
...
meson_set_control(struct asycom *asy)
{
	...
	union uart_txctrl reg;
	reg.dw = REG_READ(asy, UART_TXCTRL);
	reg.nstop = ((lcr & STB)? 1: 0);
	REG_WRITE(asy, UART_TXCTRL, reg.dw);
}

static void
meson_reset(struct asycom *asy)
{
	union uart_txctrl txctrl;
	txctrl.dw = 0;
	txctrl.txen = 1;
	txctrl.nstop = 0;
	txctrl.txcnt = 1;
	REG_WRITE(asy, UART_TXCTRL, txctrl.dw);

	union uart_rxctrl rxctrl;
	rxctrl.dw = 0;
	rxctrl.rxen = 1;
	rxctrl.rxcnt = 0;
	REG_WRITE(asy, UART_RXCTRL, rxctrl.dw);

	union uart_ie ie;
	ie.dw = 0;
	ie.txwm = 0;
	ie.rxwm = 0;
	REG_WRITE(asy, UART_IE, ie.dw);
}
...
meson_set_baud(struct asycom *asy, uint8_t bidx)
{
	...
	union uart_div reg;
	reg.dw = REG_READ(asy, UART_DIV);

	reg.div = ((asy->asy_clock + baudrate - 1) / baudrate) - 1;
	REG_WRITE(asy, UART_DIV, reg.dw);
}
...
asyattach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	...
	uint_t uart_clock = ASY_UART_CLK;
	...
	if (prom_is_compatible(ddi_get_nodeid(devi), "sifive,uart0")) {
		asy->asy_hwtype = ASYSIFIVE;
	} else {
...
async_txint(struct asycom *asy)
{
	...
	if (async->async_flags & (ASYNC_BREAK|ASYNC_OUT_SUSPEND)) {
		meson_set_icr(asy, 0, TIEN);
		return;
	}

	async_flowcontrol_sw_input(asy, FLOW_CHECK, IN_FLOW_NULL);

	if (!(async->async_flags & (ASYNC_HW_OUT_FLW|ASYNC_SW_OUT_FLW|ASYNC_STOPPED))) {
...
async_resume(struct asyncline *async)
{
	...
	if (meson_tx_is_ready(asy)) {
		if (async_flowcontrol_sw_input(asy, FLOW_CHECK, IN_FLOW_NULL))
			return;
		if (async->async_ocnt > 0 && !(async->async_flags & (ASYNC_HW_OUT_FLW|ASYNC_SW_OUT_FLW|ASYNC_OUT_SUSPEND))) {
			async->async_flags |= ASYNC_PROGRESS;
			meson_set_icr(asy, TIEN, TIEN);
		}
	}
