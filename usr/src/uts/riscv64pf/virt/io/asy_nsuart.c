...
asy_identify_chip(dev_info_t *devi, struct asycom *asy)
{
	 ...
	 */
	asy_put8(asy, FIFOR, 0x00);	/* clear */
	asy_put8(asy, FIFOR, FIFO_ON);	/* enable */
	asy_put8(asy, FIFOR, FIFO_ON | FIFORXFLSH);
						/* reset */
	...
	if (asy_fifo_test > 0 &&
		...
		asy_put8(asy, LCR,
		    DLAB);
		asy_put8(asy, DAT,
		    asy_divisor(asy, ASY_DEFAULT_BAUD) & 0xff);
		asy_put8(asy, ICR,
		    (asy_divisor(asy, ASY_DEFAULT_BAUD) >> 8) & 0xff);
		...
		for (i = 0; i < asy->asy_fifo_buf * 3; i++) {
			ret = asy_get8(asy, LSR);
			if (!(ret & RCA))
				break;	/* FIFO emptied */
			(void) asy_get8(asy, DAT); /* lose another */
		}
