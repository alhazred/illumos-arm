...
hat_tlb_inval_range(hat_t *hat, uintptr_t va, size_t len)
{
	...
	kpreempt_disable();
	const uint64_t mask = enable_hartmask & ~(CPU->cpu_hartmask);
	if (va == DEMAP_ALL_ADDR) {
		asm volatile ("sfence.vma" ::: "memory");
		sbi_remote_sfence_vma(&mask, 0, 0);
	} else {
		for (size_t i = 0; i < len; i += MMU_PAGESIZE)
			asm volatile ("sfence.vma %0, zero" ::"r"(va + i): "memory");
		sbi_remote_sfence_vma(&mask, va, len);
	}
	kpreempt_enable();
}
...
hat_flush_range(hat_t *hat, caddr_t va, size_t size)
{
	...
	while (va < endva) {
		...
		else
		{
			...
			sbi_remote_sfence_vma(&mask, (uint64_t)va, sz);
		}
		kpreempt_enable();
